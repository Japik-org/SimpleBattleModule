...
public class Vector3f extends Tuple3f implements java.io.Serializable {
 ...
 /**
  ...
  /**
   ...
   public final void rotate(Vector3f point, double angle){
       final double u=point.getX(), v=point.getY(), w=point.getZ();
       final double xPrime = u*(u*x + v*y + w*z)*(1d - Math.cos(angle))
               + x*Math.cos(angle)
               + (-w*y + v*z)*Math.sin(angle);
       final double yPrime = v*(u*x + v*y + w*z)*(1d - Math.cos(angle))
               + y*Math.cos(angle)
               + (w*x - u*z)*Math.sin(angle);
       final double zPrime = w*(u*x + v*y + w*z)*(1d - Math.cos(angle))
               + z*Math.cos(angle)
